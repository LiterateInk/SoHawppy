<img alt="SoHawppy: An awmazing API wrapper for SoHappy" src=".github/assets/banner.svg" width="100%" />

*This library **is not** affiliated with [SoHappy](https://www.so-happy.fr/) in any way.*

## What is "SoHappy" ?

TODO

## Implementations

You're currently on the `index` branch.

Since we're implementing this library in different programming languages, we have a branch for each of them.

| [JS/TS](https://github.com/LiterateInk/SoHawppy/tree/js) | Rust | Python | Swift |
| :---:  | :---: | :---: | :---: |
| Kotlin | C# | Dart | |


## License

This project is licensed under the GPL-3.0 License - see the [LICENSE.md](LICENSE.md) file for details.
